    ...
    <main class="flex-1 p-4 flex flex-col md:flex-row mx-auto w-3/4 bg-slate-100">
      <section class="shorten-section flex-1 md:mr-4 mb-4 md:mb-0">
        ...
        <form
          ...
          class="flex flex-col md:flex-row items-center space-y-4 md:space-y-0 md:w-3/5"
        >
          ...
          <input
            ...
            class="flex-1 p-3 border border-blue-500 rounded focus:outline-none focus:border-blue-700 text-gray-400"
          />
          <button
            type="submit"
            class="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-700 focus:outline-none focus:bg-blue-700 ml-2"
          >
            Shorten
          </button>
        </form>
      ...
      <aside
        class="history-sidebar w-full md:w-2/6 sd:w-full p-4 bg-gray-200 rounded-md md:text-balance"
      >
        <h2 class="text-lg font-semibold mb-4 text-gray-800">Shortened URLs History</h2>
        <ul>
          <li
            v-for="(shortLink, index) in shortenedLinks"
            :key="index"
            class="mb-2 border border-sky-500 p-2 rounded-md flex justify-between"
          >
            <div>
              <div class="flex items-center space-x-2 w-ma sd:flex-col flex-row">
                <a
                  :href="shortLink.shortened"
                  target="_blank"
                  class="text-blue-500 hover:underline p-0"
                  >{{ shortLink.shortened }}</a
                >
                <button
                  @click="copyToClipboard(shortLink.shortened)"
                  class="px-2 py-1 bg-blue-500 text-white rounded hover:bg-blue-700 focus:outline-none focus:bg-blue-700"
                >
                  Copy
                </button>
              </div>
            ...
            <button
              ...
              class="text-red-500 hover:text-red-700 focus:outline-none mr-5"
            >
